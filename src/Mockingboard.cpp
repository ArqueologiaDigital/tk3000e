...
int MBConfiguraSlot(int Slot)
{
	...
	if (MBSlot)
	{
		FrameMostraMensagemErro("Mockingboard já está configurado em outro slot");
		return ERR_EMUSO;
	}
...
void MB_Initialize()
{
	if (/*g_bDisableDirectSound || g_bDisableDirectSoundMockingboard*/0)
	{
...
void MBVerifica(WORD Endereco, BOOL write, BYTE value)
{
	if (!MBSlot)
		return;
	if ((Endereco & 0xFF00) == (0xC000 + (MBSlot << 8))) {
		if (write)
			MB_Write(0, Endereco, 1, value, 0);
		else
			MB_Read(0, Endereco, 0, value, 0);
	}
}
